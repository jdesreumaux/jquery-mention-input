...
* Modified by Julien Desreumaux
*
* Version 1.0.2
*  - Added public prepopulate function
*  - Modified addMention : No need to type triggerChar for the first mention
*  - Fixed regex if prefix doublon
*  - Fixed white-space (pre-line) in case a word is breaked at the end of a line
*  - Added trim on query and results
...
(function ($, _, undefined) {
    ...
    var defaultWidth = 'width:350px !important';

    if ($.browser.msie) {
        if (jQuery.browser.version == '8.0') {
            defaultWidth = 'width:280px !important';		
        }
    }
    else if ($.browser.safari) {
        defaultWidth = 'width:355px !important';
    }


    $('div.wrapperMention').attr('style', defaultWidth);
    ...
    var MentionsInput = function (input) {
        ...
        function updateMentionsCollection() {
          ...
          function addMention(value, id, type, isEditMode) {
            ...
            // If first mention, triggerChar is not typed yet
            if (triggerCharIndex == -1) {
                var startCaretPosition = 0;
                var currentCaretPosition = $(input).val().length;

            } else {

                // We add "$" char in case of prefix doublon
            ...
            // if prepopulating, we add trigger char between each mention

            if (typeof (isEditMode) != 'undefined' && mentionsCollection.length > 0) {
                var updatedMessageText = start + value + settings.triggerChar + end;
            }
            else updatedMessageText = start + value + end;
        ...
        return {
            ...
            prepopulate: function (name, id, type) {
               
                addMention(name, id, type, true);
